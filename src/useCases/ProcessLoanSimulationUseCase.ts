...
export class ProcessLoanSimulationUseCase {
  ...
  public async executeSmartCalculation(payload: any, internalId?: number, db?: any): Promise<{ approvedRuleId: string, currentLoanValue: number, calcResult: any, attempts: number, attemptsLogs?: any[] }> {
    ...
    while (attempts < maxAttempts && !calculationSuccess) {
      ...
      for (const rule of validRules) {
        try {
          ...
          if (db && internalId) await this.logStep(db, internalId, `step1_calc_attempt_${attempts}_rule_${rule.code}`, true, calcResult);
          break;
        } catch (error: any) {
          ...
          if (db && internalId) await this.logStep(db, internalId, `step1_calc_attempt_${attempts}_rule_${rule.code}_failed`, false, { error: errLog });
        }
      }
